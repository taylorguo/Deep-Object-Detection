...
class VGG16_Net:
	...
	def build(activation="relu", classes=17):
		...
		x = Dense(128, activation=activation, name="FC_1")(x)
		x = Dropout(0.5)(x)
		x = Dense(64, activation=activation, name="FC_2")(x)
		x = Dropout(0.5)(x)
	...
	def train(weight_path=None, load_weights=False, save_weights=True):
		...
		if not os.path.exists("models"):
			os.mkdir("models")
		best_weights = "models/best_weights_VGG16_oxflower17_val_acc_{val_acc:.3f}.h5"
		save_best_model = ModelCheckpoint(best_weights, monitor="val_acc", verbose=1, save_best_only=True)

		if load_weights == False:
			print("\t Start training ...")
			train_history = model.fit(train_d, train_l, batch_size=64, epochs=2000, verbose=1, validation_split=0.2,
